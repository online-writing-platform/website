...
export class PrismaStoryStore implements StoryStore {
    ...
    public async listPublicStories(
        ...
        const page = buildCursorPage(rows.map(mapSummary), limit, (story) => story.id);
    ...
    public async reorderChapters(
        ...
        return prisma.$transaction(async (transaction) => {
            ...
            if (
                ...
                existing.some((chapter) => !chapterIds.includes(chapter.id))
            ) {
                return null;
            }

            // Move out of the normal range first so the (storyId, position) unique
            // constraint cannot collide while positions are swapped.
