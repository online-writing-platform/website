  ...
  public async searchStories(
    ...
    const rows = await prisma.$queryRaw<SearchStoryRow[]>`
            SELECT
                ...
                COALESCE(stats."library_count", 0)::int AS "libraryCount",
                COALESCE(stats."vote_count", 0)::int AS "voteCount",
                COALESCE(stats."comment_count", 0)::int AS "commentCount",
                COALESCE(stats."qualified_views", 0)::int AS "qualifiedViews",
            ...
            WHERE story."deleted_at" IS NULL
              ...
              AND (
                  cardinality(${policy.blockedIds}::uuid[]) = 0
                  OR story."author_id" <> ALL(${policy.blockedIds}::uuid[])
              )
              AND (
                  NOT ${hasQuery}
                  OR to_tsvector(
            ...
            ORDER BY
              CASE WHEN ${filters.sort} = 'relevance' THEN GREATEST(
                  ts_rank_cd(
                      to_tsvector('simple', story."search_text"),
              ...
              CASE WHEN ${filters.sort} = 'mostRead'
                  THEN COALESCE(stats."qualified_views", 0)
              END DESC,
              CASE WHEN ${filters.sort} = 'mostVoted'
                  THEN COALESCE(stats."vote_count", 0)
              END DESC,
              CASE WHEN ${filters.sort} = 'newest'
