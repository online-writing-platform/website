...
import {
    ...
    normalizeTagName,
} from "../../../shared/text/slug.js";

import { assertPublishedStoryStatus } from "../publishing/story.policy.js";
import type {
    CreateStoryInput,
    StoryDetail,
    StoryPage,
...
export class StoryService {
    ...
    public async create(authorId: string, input: CreateStoryInput): Promise<StoryDetail> {
        if (input.genreSlug && !(await this.store.genreExists(input.genreSlug))) {
    ...
    public async update(
        ...
        if (
            ...
            !(await this.store.genreExists(input.genreSlug))
        ) {
            throw AppError.badRequest("The selected genre is invalid.", "INVALID_GENRE");
        }
    ...
    public listMine(
        ...
        return this.store.listOwnedStories(authorId, cursor, limit);
    }

    public listGenres(): Promise<Array<{ slug: string; name: string }>> {
        return this.store.listGenres();
    }
}
