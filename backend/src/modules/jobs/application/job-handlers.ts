...
import type { ClaimedJob, ClaimedOutbox } from "../infrastructure/prisma-work-queue.js";
...
export async function handleJob(job: ClaimedJob, now = new Date()): Promise<void> {
            ...
            if (result.count === 1) {
                await transaction.outboxMessage.create({
                    data: {
                        ...
                        aggregateType: "STORY",
                        aggregateId: storyId,
                        payload: { storyId, publishedAt: now.toISOString() },
                    },
                });
            }
        });
            ...
            if (result.count === 1) {
                await transaction.outboxMessage.create({
                    data: {
                        ...
                        payload: { chapterId, publishedAt: now.toISOString() },
                    },
                });
            }
        });
    ...
    if (job.type === "RECONCILE_STORY_COUNTERS") {
        const storyId = stringField(payload, "storyId");
        const [votes, comments, library, qualifiedViews, readers] = await Promise.all([
            prisma.chapterVote.count({ where: { chapter: { storyId, deletedAt: null } } }),
            prisma.comment.count({ where: { chapter: { storyId }, status: "ACTIVE" } }),
            prisma.libraryEntry.count({ where: { storyId } }),
            prisma.readSignal.count({ where: { storyId } }),
            prisma.readSignal.groupBy({ by: ["userId"], where: { storyId, userId: { not: null } } }),
        ]);
        await prisma.storyStats.upsert({
            where: { storyId },
            create: {
                storyId, voteCount: votes, commentCount: comments, libraryCount: library,
                qualifiedViews, authenticatedReaders: readers.length,
            },
            update: {
                voteCount: votes, commentCount: comments, libraryCount: library,
                qualifiedViews, authenticatedReaders: readers.length,
            },
        });
        return;
...
export async function handleOutbox(message: ClaimedOutbox): Promise<void> {
    if (message.eventType === "STORY_PUBLISHED") {
        ...
        await prisma.$transaction([
            prisma.job.upsert({
                where: { dedupeKey: `reconcile-story:${message.id}` },
                create: {
                    type: "RECONCILE_STORY_COUNTERS",
                    dedupeKey: `reconcile-story:${message.id}`,
                    payload: { storyId: story.id, eventId: message.id },
                },
                update: {},
            }),
            ...story.author.followers.map(({ followerId }) =>
                prisma.notification.upsert({
                    where: { dedupeKey: `story-published:${message.id}:${followerId}` },
                    create: {
    ...
    if (message.eventType === "CHAPTER_PUBLISHED") {
            ...
            select: {
                ...
                story: {
                    select: {
                        ...
                        title: true,
                        authorId: true,
                        author: { select: { followers: { select: { followerId: true } } } },
                    },
                },
            },
        });
        if (!chapter) return;
        await prisma.$transaction(
            chapter.story.author.followers.map(({ followerId }) =>
                prisma.notification.upsert({
                    where: { dedupeKey: `chapter-published:${message.id}:${followerId}` },
                    create: {
                        recipientId: followerId,
