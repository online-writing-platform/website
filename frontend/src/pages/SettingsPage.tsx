...
export default function SettingsPage() {
  ...
  async function updatePreference(next: Partial<Preferences>): Promise<void> {
    ...
    setMessage(null);
    try {
      const response = await request<PreferencesResponse>(
        "/api/v1/preferences",
        {
          method: "PATCH",
  ...
  async function changeUsername(
    ...
    try {
      ...
      setNewUsername("");
      setMessage(
        "نام کاربری تغییر کرد. برای دیدن اطلاعات تازه صفحه را بازخوانی کنید.",
      );
    } catch (cause) {
      setError(getErrorMessage(cause));
    }
  ...
  async function resendVerification(): Promise<void> {
    try {
      await request("/api/v1/auth/email-verification/resend", {
        method: "POST",
      });
    ...
    <main className="page-shell settings-page">
      ...
      <section className="surface settings-profile">
        <header className="settings-profile__header">
          ...
          <p>اطلاعاتی که در صفحه عمومی پروفایل شما نمایش داده می‌شود.</p>
        </header>
        ...
        >
          <label className="settings-profile__field">
          ...
          <label className="settings-profile__field">
            ...
            <small>{bio.length.toLocaleString("fa-IR")} از ۵۰۰ نویسه</small>
          </label>

          <label className="settings-profile__field">
            <span>نشانی تصویر پروفایل</span>

            <input
              className="settings-profile__url"
              type="url"
              maxLength={2048}
              value={avatarUrl}
      ...
      <section className="surface">
        ...
        <div className="settings-security-grid">
          ...
          <form
            className="stack-form"
            onSubmit={(event) => void changeUsername(event)}
          >
      ...
      <section className="surface danger-zone">
        <h2>حذف حساب</h2>
        <p>
          حذف حساب یک عملیات حساس است و بعد از تأیید رمز عبور، نشست‌های فعال لغو
          می‌شوند.
        </p>
